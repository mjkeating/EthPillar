...
#
# Node-Install: Standalone Lodestar BN + Standalone Lodestar VC + Besu EL + MEVboost
# Quickstart :: Typescript Clients :: Docker-free
...
cl_ver = lodestar.download_lodestar(eth_network)
cl_path = ""
val_path = ""

if not VALIDATOR_ONLY:
    fee_params = f'--suggestedFeeRecipient={FEE_RECIPIENT_ADDRESS}'
    mev_params = '--builder --builder.urls http://127.0.0.1:18550' if MEVBOOST_ENABLED else ''
    cl_path = lodestar.install_lodestar_bn(
...
if VALIDATOR_ENABLED:
    fee_params = f'--suggestedFeeRecipient={FEE_RECIPIENT_ADDRESS}'
    mev_params = '--builder' if MEVBOOST_ENABLED else ''
    bn_addr = f'--beaconNodes={bn_address}' if VALIDATOR_ONLY else f'--beaconNodes=http://{CL_IP_ADDRESS}:{args.cl_rest_port}'
