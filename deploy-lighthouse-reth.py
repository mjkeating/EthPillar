...
if not args.network and not args.skip_prompts:
    index = SelectionMenu.get_selection(valid_networks, title='Validator Install Quickstart :: CoinCashew.com', subtitle='Installs Reth EL / Lighthouse BN / Lighthouse VC / MEVboost\nSelect Ethereum network:')
...
if not VALIDATOR_ONLY:
    reth_ver, reth_path = reth.download_and_install_reth(
        eth_network, args.el_p2p_port, EL_P2P_PORT_2, args.el_rpc_port, 
...
if not VALIDATOR_ONLY:
    mev_params = f'--builder http://127.0.0.1:18550' if MEVBOOST_ENABLED else ''
    cl_path = lighthouse.install_lighthouse_bn(
        eth_network, sync_url, args.jwtsecret, 
        args.cl_rest_port, args.cl_p2p_port, CL_P2P_PORT_2, args.cl_max_peers,
...
common.finish_install(
    ...
    "reth", reth_ver, reth_path,
    "lighthouse", cl_ver, cl_path,
