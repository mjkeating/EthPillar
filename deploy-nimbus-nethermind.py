...
#
# Node-Install: Standalone Nimbus BN + Standalone Nimbus VC + Nethermind EL + MEVboost
# Quickstart :: Docker-free
...
if not VALIDATOR_ONLY:
    # Nethermind specific sync params...
    sync_params = globals().get(f"{eth_network}_nethermind_sync_parameters", '')
    nm_ver, nm_path = nethermind.download_and_install_nethermind(
        eth_network, args.el_p2p_port, args.el_rpc_port, 
        args.el_max_peers, args.jwtsecret, sync_parameters=sync_params
    )

cl_ver = nimbus.download_nimbus(eth_network)
cl_path = ""
val_path = ""

if not VALIDATOR_ONLY:
    fee_params = f'--suggested-fee-recipient={FEE_RECIPIENT_ADDRESS}'
    mev_params = '--payload-builder=true --payload-builder-url=http://127.0.0.1:18550' if MEVBOOST_ENABLED else ''
    cl_path = nimbus.install_nimbus_bn(
        eth_network, args.jwtsecret, 
...
if VALIDATOR_ENABLED:
    fee_params = f'--suggested-fee-recipient={FEE_RECIPIENT_ADDRESS}'
    mev_params = '--payload-builder=true' if MEVBOOST_ENABLED else ''
    bn_addr = f'--beacon-node={bn_address}' if VALIDATOR_ONLY else f'--beacon-node=http://{CL_IP_ADDRESS}:{args.cl_rest_port}'
...
common.finish_install(
    ...
    "nethermind", nm_ver, nm_path,
    "nimbus", cl_ver, cl_path,
