...
if not args.network and not args.skip_prompts:
    index = SelectionMenu.get_selection(valid_networks, title='Validator Install Quickstart :: CoinCashew.com', subtitle='Installs Erigon EL / Caplin BN / Lighthouse VC / MEVboost\nSelect Ethereum network:')
...
if not VALIDATOR_ONLY:
    mev_params = f'--caplin.mev-relay-url=http://127.0.0.1:18550' if MEVBOOST_ENABLED else ''
    erigon_ver, erigon_path = erigon.download_and_install_erigon(
        eth_network, args.el_p2p_port, args.el_rpc_port, args.el_max_peers,
        args.jwtsecret, args.cl_p2p_port, args.cl_rest_port, args.cl_max_peers,
        sync_url, mev_parameters=mev_params
    )

# Download Lighthouse for validator regardless if VALIDATOR_ENABLED
cl_ver = lighthouse.download_lighthouse(eth_network)
val_path = ""

if VALIDATOR_ENABLED:
    fee_params = f'--suggested-fee-recipient={FEE_RECIPIENT_ADDRESS}'
    mev_params = '--builder-proposals' if MEVBOOST_ENABLED else ''
    # Integrated Erigon-Caplin: BN is local
    bn_addr = f'--beacon-nodes={bn_address}' if VALIDATOR_ONLY else f'--beacon-nodes=http://127.0.0.1:{args.cl_rest_port}'
...
common.finish_install(
    ...
    "erigon-caplin", erigon_ver, erigon_path,
    "", cl_ver, "", # Integrated client, no separate consensus service path for BN
