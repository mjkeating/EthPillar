...
def download_lighthouse(eth_network):
    ...
    Returns:
        lighthouse_version: The version string of the downloaded Lighthouse
    """
    ...
    # Remove the lighthouse.tar.gz file
    os.remove(tar_filename)

    return lighthouse_version

def install_lighthouse_bn(eth_network, sync_url, jwtsecret_path, 
                          cl_rest_port, cl_p2p_port, cl_p2p_port_2, cl_max_peer_count,
                          network_override=None, mev_parameters=''):
    """Install Lighthouse beacon node service."""
    ...
    service_content = generate_lighthouse_bn_service(
        ...
        cl_p2p_port, cl_p2p_port_2, cl_max_peer_count, network_override, mev_parameters
    )
...
def install_lighthouse_vc(eth_network, graffiti, beacon_node_address,
                          fee_parameters='', mev_parameters='', network_override=None):
    """Install Lighthouse validator client service."""
    # Create User and directories
    ...
    service_content = generate_lighthouse_vc_service(
        eth_network, graffiti, beacon_node_address, fee_parameters, mev_parameters, network_override
    )
